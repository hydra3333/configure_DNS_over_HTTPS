...
**Version:** 1.2.2 — 2025‑09‑11 (Australia/Adelaide)  
...
* If either fails -> **degrade to Allow**: set `automatic` (leave templates empty) and **warn**.
...
8. **Firefox DoH**:
   ...
   * If `-PrintDiagnostics`: show additional canary result details (normal info printing will show brioef result), any policies detected, and `policies.json` presence (warn if policy overrides user.js).
9. **Summary**:
...
## 12) **Potential Gaps to review and if they exist then address**
